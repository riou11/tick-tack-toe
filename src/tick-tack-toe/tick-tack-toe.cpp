...
public:
    ...
    bool put(int x, int y) {
        if (x < 0 || BOARD_SIZE <= x || y < 0 || BOARD_SIZE <= y) return false; // 盤面外
...
int AI_nega_max::evaluate(Board& b, Mass::status current, int& best_x, int& best_y) {
    ...
    for (int y = 0; y < Board::BOARD_SIZE; y++) {
        for (int x = 0; x < Board::BOARD_SIZE; x++) {
            ...
            m.setStatus(current);
            int dummy_x, dummy_y;
            int score = -evaluate(b, next, dummy_x, dummy_y);
...
void show_start_message() {
    ...
    std::cout << std::endl;
    std::cout << "input position likes a  1" << std::endl;
    std::cout << "========================" << std::endl;
}
